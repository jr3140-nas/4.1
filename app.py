...
def prepare_report_data(time_df: pd.DataFrame, selected_dates) -> Dict[str, Any]:
    df = time_df.copy()

    # Backward-compatible: accept single date or list
    ...
    if "Production Date" in df.columns and df["Production Date"].notna().any():
        if pd.notna(selected_date):
            df = df[pd.to_datetime(df["Production Date"], errors="coerce").dt.date.isin(selected_dates)]
...
if "Production Date" in time_df.columns and not pd.Series(time_df["Production Date"]).dropna().empty:
    ...
    selected_labels = st.multiselect("Select Production Date(s)", options=date_labels, default=[date_labels[-1]])
    selected_dates = [label_to_date[l] for l in selected_labels]
else:
    selected_labels = ["All Data"]
    selected_dates = [pd.NaT]
    st.info("No valid 'Production Date' found — showing all data.")

report = prepare_report_data(time_df, selected_dates)

with st.sidebar:
    st.subheader("Export")
    st.download_button(
        "Generate & Download PDF (Landscape)",
        data=build_pdf(report, ', '.join(selected_labels)),
        file_name=f"workorder_report_{(','.join(selected_labels)).replace('/', '-')}.pdf",
        mime="application/pdf",
    )

st.markdown(f"### Report for {', '.join(selected_labels)}")
