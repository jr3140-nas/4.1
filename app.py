# WorkOrderReport4.0 — Streamlit app + Crew filter (robust header/date handling)
# Enhancements in this build:
# - Header normalization (strip, collapse spaces, remove non-breaking spaces)
# - Case-insensitive, punctuation-insensitive synonym mapping
# - More tolerant Production Date parsing (strings, datetimes, Excel serials)
# - Crew filter as requested (default All Crews)
...
def _clean_header(s: str) -> str:
    # remove non-breaking spaces and collapse internal whitespace
...
def _resolve_hours_column(df: pd.DataFrame) -> pd.DataFrame:
    ...
    aliases = [
        ...
        "sumofhours", "tot_hrs", "tot hours", "manhours", "man hours", "labor hours", "labour hours"
    ]
...
# Robust Production Date parsing
def _parse_production_date(series: pd.Series) -> pd.Series:
    s = series.copy()
    # Try regular parse (handles strings and datetimes)
    dt = pd.to_datetime(s, errors="coerce", infer_datetime_format=True)
    if dt.notna().sum() >= s.notna().sum() * 0.5:
        return dt.dt.date

    # If mostly numbers (Excel serials)
    numeric = pd.to_numeric(s, errors="coerce")
    if numeric.notna().sum() >= s.notna().sum() * 0.5:
        try:
    ...
    # Give it one last try with dayfirst
    dt3 = pd.to_datetime(s, errors="coerce", dayfirst=True, infer_datetime_format=True)
    return dt3.dt.date
...
def load_timeworkbook(xlsx_file) -> pd.DataFrame:
    # Read the first sheet; if headers look empty, try header=None heuristic.
    ...
    if "Production Date" in df.columns:
        df["Production Date"] = _parse_production_date(df["Production Date"])
    elif "Date" in df.columns:
        df["Production Date"] = _parse_production_date(df["Date"])
    else:
        df["Production Date"] = pd.NaT

    # Cost Center from header or column N (index 13) if present
    ...
    # Normalize Crew column name if present in variants
    crew_variants = ["Crew", "crew", "CREW", "Shift/Crew", "Shift - Crew", "Shift_Crew", "ShiftCrew"]
    for variant in crew_variants:
        if variant in df.columns and "Crew" not in df.columns:
            df.rename(columns={variant: "Crew"}, inplace=True)
            break

    return df

def prepare_report_data(time_df: pd.DataFrame, selected_date, selected_crew: str | None) -> Dict[str, Any]:
    df = time_df.copy()
    ...
    # Crew filter
    if selected_crew and selected_crew != "All Crews" and "Crew" in df.columns:
        df = df[df["Crew"].astype(str) == selected_crew]
...
# Production Date select
if "Production Date" in time_df.columns and not pd.Series(time_df["Production Date"]).dropna().empty:
    dates = sorted(pd.to_datetime(time_df["Production Date"], errors="coerce").dt.date.dropna().unique())
    if len(dates) == 0:
...
# Crew filter select (NEW)
selected_crew = None
if "Crew" in time_df.columns:
    df_for_options = time_df.copy()
    if pd.notna(selected_date):
        df_for_options = df_for_options[pd.to_datetime(df_for_options["Production Date"], errors="coerce").dt.date == selected_date]
    crews = sorted([c for c in df_for_options["Crew"].dropna().astype(str).unique() if str(c).strip() != ""])
    crew_options = ["All Crews"] + crews
    selected_crew = st.selectbox("Filter by Crew", options=crew_options, index=0, help="Default shows all crews.")
else:
    selected_crew = "All Crews"

report = prepare_report_data(time_df, selected_date, selected_crew)
